...
> 이 문서는 [tasks.md](../resume-chat/tasks.md)에서 Frontend 개발 작업만 분리한 체크리스트입니다.
...
- [x] WebSocket 라이브러리 설치 (@stomp/stompjs, sockjs-client)
- [x] WebSocket 클라이언트 유틸리티 생성 (/shared/api/websocket.ts)
  - [x] SockJS 연결 설정
  - [x] STOMP 클라이언트 초기화
  - [x] 자동 재연결 로직 (5초 간격)
  - [x] 구독(subscribe) 함수
  - [x] 메시지 발행(publish) 함수
- [x] ChatPage에 WebSocket 통합
  - [x] useEffect로 연결 관리
  - [x] 실시간 메시지 수신 처리
  - [x] 메시지 전송 시 WebSocket 사용
  - [x] 연결 상태 표시 UI
  - [x] cleanup 함수로 연결 해제
...
**문서 버전**: 1.0
**최종 업데이트**: 2026-03-10
**관련 문서**: [tasks.md](../resume-chat/tasks.md), [plan.md](../resume-chat/plan.md), [spec.md](../resume-chat/spec.md)
